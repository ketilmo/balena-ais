...
parse_outputs() {
    ...
    echo "Scanning for ${prefix} configurations..." >&2
    
    for var in $(compgen -e | grep "^${prefix}_" | sort); do
        output_value="${!var}"
        
        if [ -z "$output_value" ]; then
            echo "Warning: $var is empty, skipping" >&2
            continue
        fi
...
#   AIS_OUTPUT_MQTT_BROKER=mqtt://username:password@127.0.0.1:1883|client_id aiscatcher|qos 0|topic data/ais
...
AIS_CONFIG="-d $AIS_DEVICE -gr RTLAGC on TUNER auto BIASTEE off -a $AIS_TUNER_BANDWIDTH -p $AIS_DEVICE_CORRECTION_PPM -o $AIS_OUTPUT_MODE -v $AIS_VERBOSE_MODE -M $AIS_ADDITIONAL_METADATA -N $AIS_WEB_PORT REALTIME on STATION $AIS_STATION_NAME LAT $LAT LON $LON SHARE_LOC on $AIS_SHARING $AIS_OUTPUT_TCPS $AIS_OUTPUT_TCPC $AIS_OUTPUT_UDP $AIS_OUTPUT_HTTP $AIS_OUTPUT_MQTT"
